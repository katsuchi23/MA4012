...
const int EAST_ALIGN_STABLE_SAMPLES   = 3;
...
void alignToEastCCW(int timeoutMs) {
  ...
  // Strict alignment: finish only when east=0 and others are 1 (stable samples).
  while (stableCount < EAST_ALIGN_STABLE_SAMPLES) {
    if (isFacingEast()) {
      stableCount++;
      stopDrive();
      wait1Msec(20);
